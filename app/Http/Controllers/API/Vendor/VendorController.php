...
class VendorController extends Controller
{
    /* login api */
     ...
     { 
        ...
        $user = User::where('email',$request->email)->get(['id','status','is_blocked']);
        ...
        $notification_data = [
                "to" => $dtoken,
                    "notification" => [
                        "icon"=>"http://student-vouchers.auspicioussoft.com/public/images/icon2.png",
                        "title" => "Uniplusco",
    ...
    { 
        ...
        else 
        {
            ...
            if($request->image)
            {
                ...
                $user_meta->image = $filename;
                
            }

            $user_meta->save();
            
              $credentials =['email'=>$user->email,'password'=>$user->plane_password];
    ...
    { 
        ...
        // if(sizeof($user_id))
        // {
        //     $optdata = User_otp::where('email',$request->email)->where('user_id',$user_id[0]->id)->get();
    ...
    public function user(Request $request) 
    {
        ...
        ->select(['user_metas.user_id','users.first_name','users.last_name','users.email','users.address','user_metas.phone','user_metas.discription'])
        ->where('user_type',3)->find($id);
        ...
        if($user)
        {
            ...
            $data['message']        =   'Vendor Data Fetched';
            $data['data']      =         $user;  
        }
        else
        {
    ...
    {   
        ...
        if(sizeof($optdata))
        {
            ...
            return response()->json(['message' => 'Otp sent on your mail!','otp'=>$otp ], 200);
    ...
    public function edituser(Request $request) 
    {
        ...
        $user = User::join('user_metas','users.id','=','user_metas.user_id')
                ->select(['user_metas.id','users.first_name','users.last_name','users.address','user_metas.phone','user_metas.discription','user_metas.image','user_metas.user_id'])
                ->where('user_type',3)->find($id);
        // print_r($id);
        // die;
        $first_name = $request->first_name;
        ...
        if($request->phone)
        {
            $user_meta= User_meta::where('user_id',$id)->update(['phone'=>$phone]);
        }

        else
        {
            $user_meta= User_meta::where('user_id',$id)->update(['phone'=>$user->phone]);
        }

        if($request->discription)
        {
            $user_meta= User_meta::where('user_id',$id)->update([ 'discription'=>$discription]);
        }

        else
        {
            $user_meta= User_meta::where('user_id',$id)->update(['discription'=>$user->discription]);
        }
    ...
    public function notifications(Request $request)
    {
       ...
       $notification = Notification::where('user_id',$device_token[0]->user_id)
                          ->where('read_status','0')
                          ->orderBy('id','DESC')->get(['id','subject','message','image']);
        if($notification)
        {
            $data['status_code']    =   1;
            $data['status_text']    =   'Success';             
